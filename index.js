        ...
        try {
                  ...
                  const withThumbs = await Promise.all(response.data.files.map(async (f) => {
                              const isImg = (f.mimeType && f.mimeType.startsWith('image/')) ||
                                            (f.name && /\.(jpg|jpeg|png|gif|webp|bmp)$/i.test(f.name));
                              if (isImg) {
                                            try {
                                                            ...
                                                            return { ...f, thumb: `data:${f.mimeType || 'image/jpeg'};base64,${b64}` };
                                            } catch (e) { return f; }
                              }
                              return f;
                  }));
...
// Proxy — MOV/AVI se serviraju kao video/mp4 da Chrome moze da ih reproducira
app.get('/api/drive/file/:fileId', async (req, res) => {
        try {
                  ...
                  // MOV i AVI — servirati kao video/mp4 da Chrome moze da reproducira
...
app.post('/api/captions/generate', async (req, res) => {
        ...
        try {
                  const r = await fetch('https://api.captions.ai/api/caption/create', {
                              ...
                              body: JSON.stringify({ videoUrl, language: language || 'en' }),
                  });
                  const data = await r.json();
                  if (!r.ok) throw new Error(data.message || JSON.stringify(data));
                  res.json(data);
